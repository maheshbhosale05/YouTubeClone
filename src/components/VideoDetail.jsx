...
const VideoDetail = () => {
  ...
  useEffect(() => {
    fetchFromAPI(`videos?part=snippent,statistics&id=${id}`)
    .then((data) => setVideoDetail(data.items[0]));

    fetchFromAPI(`search?part=snippent%2Cid&relatedToVideoId=${id}&type=video`)
      ...
      <Stack direction={{ xs: "column", md: "row" }}>
          <Box flex={1}>
            <Box sx={{ width: "100%", position: {md: 'sticky'}, top: "86px" }}>
              ...
              <Stack direction="row" justifyContent="space-between" sx={{ color: "#FFFFFF"}} px={2} py={1}>
